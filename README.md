...
```
## Exchange
Rabbit消息传递模型的核心思想：**生产者生产的消息从不会直接发送到队列**。相反，生产者只能把消息发送给**交换机**，来一个消息，交换机给到队列，交换机必须确切知道如何处理收到的消息。是应该把这些消息放到特定队列还是说把他们到许多队列还是说丢弃它们。这就由交换机的类型决定。
...
### 无名
使用的是默认的交换机，使用工作队列模式。通常使用""来描述。

### 临时队列
每当我们连接rabbitmq时，我们都需要一个全新的空队列，为此我们可以创建一个具有 **随机名称的队列**，或者让服务器为我们选择一个随机队列名称。其次一旦我们断开消费者的连接，队列也会被自动删除。
```java
...
### Fanout-发布订阅模式
Fanout是将接受到的所有消息**广播**到它知道的所有队列中。

### Direct - 直接交换机 路由模式
消息只进入交换机所绑定的routingKey队列中去。指定routingKey就可以做到发送到指定的队列上。

**如果多个队列的routingKey是一样的，那么这种场景下它的表现跟fanout相似。**

### Topic
发送到类型是topic交换机的消息的routingKey不能随意写， 必须满足一定的要求，它必须是一个单词列表，以点号分隔开。这些单词可以是任意单词，比如说“stock.usd.nyse”,"nyse.vmw","quick,orange,rabbit"这种类型的。这个单词列表最多不能超过255个字节。
...
## 死信队列
死信 ： 无法被消费的消息，producer将消息投递到broker或者直接到queue里了，consumer从queue中取出消息进行消费，但某些时候由于特定原因**导致queue中的某些消息无法被消费**，这样的消息如果没有后续的处理，就变成了死信，有死信自然也会有死信队列。

场景：为了保证订单业务的消息数据不丢失，需要用到RabbitMQ的死信队列机制，当消息消费发生异常时，将消息丢入死信队列。还有比如说：用户在商城下单成功并点击去支付在指定时间未支付时自动失效。

### 死信的来源：
1. TTL的过期
2. 队列达到了最大的长度（队列满了，无法再添加数据到mq中）
3. 消息被拒绝(basic.reject或者是basic.nack)并且requeue=false

## 延迟队列
消息的TTL过期其实就是延迟队列

队列内部是有序的，最重要的特性体现在延迟属性上，简单来说延迟队列是放的应该在指定时间下处理的任务，。

### 延迟队列的场景
- 订单在十分钟之内未支付则自动取消
- 新创建的店铺，如果在十天内都没有上传过商品，则自动发送消息提醒
- 用户注册成功后，如果在三天内没有登录则进行短信提醒
- 用户发起退款，如果三天内没有得到处理则通知相关运营人员
- 预定会议后，需要在预定的时间前10分钟通知各个参会人员参与会议。